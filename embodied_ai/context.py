...
class AgentContext:
    ...
    def record(self, observation: dict[str, Any], action: dict[str, Any], events: list[dict[str, Any]]) -> None:
        ...
        self.facts.update(event.get("message", "") for event in events if event.get("type") in {"DoorLocked", "NpcSpoke", "ItemObserved"})
