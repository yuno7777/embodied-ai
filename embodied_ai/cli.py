...
def main():
    ...
    if args.cmd=='run':
        ...
        export_parquet([{**record,"observation":json.dumps(record["observation"]),"next_observation":json.dumps(record.get("next_observation")),"research_snapshot":json.dumps(record.get("research_snapshot")),"next_research_snapshot":json.dumps(record.get("next_research_snapshot")),"agent_context":json.dumps(record["agent_context"]),"events":json.dumps(record["events"]),"chosen_action":json.dumps(record["chosen_action"]),"metrics":json.dumps(record["metrics"])} for record in result.records],directory/f'{result.run_id}.parquet')
        ...
        rows=[{'partition':partition, 'outcome':episode['terminal_reason'], 'observation_mode':args.observation_mode, 'generator_config':generator_config, 'generator_config_fingerprint':config_fingerprint, **episode} for partition,episodes in evaluated.items() for episode in episodes]
