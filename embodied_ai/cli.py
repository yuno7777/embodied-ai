...
def main():
    ...
    generalize=sub.add_parser('generalize'); generalize.add_argument('--provider',choices=provider_choices,default='scripted'); generalize.add_argument('--train-start',type=int,default=0); generalize.add_argument('--train-count',type=int,default=5); generalize.add_argument('--validation-start',type=int,default=8_000); generalize.add_argument('--validation-count',type=int,default=2); generalize.add_argument('--test-start',type=int,default=9_000); generalize.add_argument('--test-count',type=int,default=2); generalize.add_argument('--concurrency',type=int,default=1); generalize.add_argument('--generator-config',type=Path); generalize.add_argument('--train-generator-config',type=Path); generalize.add_argument('--validation-generator-config',type=Path); generalize.add_argument('--test-generator-config',type=Path); generalize.add_argument('--output',type=Path,default=ROOT/'data'/'exports'/'generalization'); generalize.add_argument('--server-url')
    train_q=sub.add_parser('train-tabular'); train_q.add_argument('--seed-start',type=int,default=0); train_q.add_argument('--episodes',type=int,default=20); train_q.add_argument('--max-steps',type=int,default=128); train_q.add_argument('--learning-rate',type=float,default=.2); train_q.add_argument('--discount',type=float,default=.95); train_q.add_argument('--epsilon',type=float,default=.2); train_q.add_argument('--checkpoint',type=Path,default=ROOT/'data'/'checkpoints'/'tabular_q.json'); train_q.add_argument('--server-url')
    eval_q=sub.add_parser('evaluate-tabular'); eval_q.add_argument('--checkpoint',type=Path,required=True); eval_q.add_argument('--seed-start',type=int,default=9000); eval_q.add_argument('--episodes',type=int,default=10); eval_q.add_argument('--max-steps',type=int,default=128); eval_q.add_argument('--server-url')
    ...
    if args.cmd=='run':
        ...
        try:
            ...
            episodes=train_tabular_q(lambda: EmbodiedEnv(EmbodiedEnvConfig(server_url=args.server_url)),policy,seeds,args.max_steps,lambda seed: {"generated_world":{"seed":seed}})
        except ValueError as error:
            p.error(str(error))
        checkpoint=policy.save(args.checkpoint)
        print(json.dumps({"policy":"tabular_q","episodes":len(episodes),"checkpoint":str(checkpoint),"mean_reward":sum(item["total_reward"] for item in episodes)/len(episodes),"terminal_reasons":{reason:sum(item["terminal_reason"]==reason for item in episodes) for reason in sorted({item["terminal_reason"] for item in episodes})}},indent=2))
        ...
        try:
            ...
            seeds=list(range(args.seed_start,args.seed_start+args.episodes))
            episodes=evaluate_tabular_q(lambda: EmbodiedEnv(EmbodiedEnvConfig(server_url=args.server_url)),policy,seeds,args.max_steps,lambda seed: {"generated_world":{"seed":seed}})
        except (OSError, ValueError, json.JSONDecodeError) as error:
            p.error(str(error))
        print(json.dumps({"policy":"tabular_q","checkpoint":str(args.checkpoint),"episodes":episodes,"success_rate":sum(item["terminal_reason"]=="escaped" for item in episodes)/len(episodes),"mean_reward":sum(item["total_reward"] for item in episodes)/len(episodes)},indent=2))
