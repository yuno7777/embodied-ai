...
- The Python `EmbodiedEnv` implements reset/step semantics over the Rust service, including generated-world, named train/validation/test distribution, and reward-profile requests. `distribution` is an authoritative generated-world partition assertion; the older `world_partition` spelling remains compatible.
