...
The canonical protocol version is `1`. A provider receives an `Observation` and returns one `ActionRequest`; it never receives a `WorldSnapshot` or a mutation channel. `StepResult` contains the new filtered observation, reward, terminal information and structured events. `WorldSnapshot` is researcher-only.
