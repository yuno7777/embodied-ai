...
type AgentMetadata = { confidence?: number; value_estimate?: number; policy_entropy?: number; planner?: { name: string; expanded_nodes?: number; planning_time_ms?: number } };
type DecisionRecord = { step: number; action: { type: string; direction?: string; target_id?: string; item_id?: string }; decision_summary: string; provider: string; model?: string; latency_ms?: number; agent_metadata?: AgentMetadata | null };
...
export default function Observer({ initialRunId }: { initialRunId?: string }) {
  ...
  return <main className={theme}>
    ...
    <section><div className="panel"><span>RESEARCHER VIEW</span><div className="map" style={{ gridTemplateColumns: "repeat(" + width + ",1fr)" }}>
      ...
      <div className="panel telemetry"><span>AGENT DECISION</span>{agentDecision ? <><p>Step {agentDecision.step} · {agentDecision.provider}{agentDecision.model ? " / " + agentDecision.model : ""}{agentDecision.latency_ms !== undefined ? " · " + agentDecision.latency_ms + " ms" : ""}</p><b>{agentDecision.action.type.toUpperCase()}{agentDecision.action.direction ? " " + agentDecision.action.direction.toUpperCase() : agentDecision.action.target_id ? " " + agentDecision.action.target_id : agentDecision.action.item_id ? " " + agentDecision.action.item_id : ""}</b><p>{agentDecision.decision_summary || "No decision summary supplied."}</p>{agentDecision.agent_metadata && <p><small>POLICY TELEMETRY · {[agentDecision.agent_metadata.confidence !== undefined ? "confidence " + agentDecision.agent_metadata.confidence : null, agentDecision.agent_metadata.value_estimate !== undefined ? "value " + agentDecision.agent_metadata.value_estimate : null, agentDecision.agent_metadata.policy_entropy !== undefined ? "entropy " + agentDecision.agent_metadata.policy_entropy : null, agentDecision.agent_metadata.planner ? "planner " + agentDecision.agent_metadata.planner.name + (agentDecision.agent_metadata.planner.expanded_nodes !== undefined ? " / " + agentDecision.agent_metadata.planner.expanded_nodes + " nodes" : "") + (agentDecision.agent_metadata.planner.planning_time_ms !== undefined ? " / " + agentDecision.agent_metadata.planner.planning_time_ms + " ms" : "") : null].filter((value): value is string => value !== null).join(" · ")}</small></p>}</> : <p className="empty">No provider decision recorded yet.</p>}</div>
