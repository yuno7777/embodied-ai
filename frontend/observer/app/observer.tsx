...
type Observation = { step: number; observation_mode: string; current_room_id?: string; visible_cells: { relative_position: Pos; terrain: string; entities: { id: string; type: string; state?: string; name?: string }[] }[]; recent_events: string[]; perception_note?: string };
...
type Replay = { snapshot: Snapshot; events: RunEvent[]; timeline?: Snapshot[]; observations?: Observation[]; decisions?: DecisionRecord[] };
...
export default function Observer({ initialRunId }: { initialRunId?: string }) {
  ...
  const [agentExports, setAgentExports] = useState<{ jsonl?: string; parquet?: string }>({});
  ...
  useEffect(() => {
    ...
    const refreshAgentStatus = async () => {
      try {
        ...
        const status = await response.json() as { status?: string; exports?: { jsonl?: string; parquet?: string } };
  ...
  async function startAgent() {
    ...
    setRun(data.run_id); setConnection("reconnecting"); setState(undefined); setAgentObservation(undefined); setAgentDecision(undefined); setFrames([]); setObservationFrames([]); setDecisionFrames([]); setPlaying(false); setEvents([notice("Provider run accepted: " + provider + ".")]); setPaused(false); setAgentRunStatus(data.status); setAgentExports({}); setControlMode("agent"); setLastAction("Waiting for " + provider + " to choose the first simulated action."); void refreshRuns();
  }
  const loadReplay = useCallback(async (id: string) => {
    ...
    setRun(id); setConnection("idle"); setState(lastFrame); setAgentObservation(replayObservations[timeline.length - 1]); setAgentDecision(replayDecisions.filter((decision) => decision.step <= lastFrame.step).at(-1)); setFrames(timeline); setObservationFrames(replayObservations); setDecisionFrames(replayDecisions); setFrameIndex(timeline.length - 1); setPlaying(false); setEvents([...replay.events].reverse()); setPaused(false); setAgentRunStatus(undefined); setAgentExports({}); setControlMode("replay"); setLastAction("Loaded persisted replay.");
  }, []);
  useEffect(() => {
