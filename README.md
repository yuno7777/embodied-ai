...
`generalize-tabular` trains once on an explicit procedural train split, freezes the checkpoint, then evaluates that same policy over train, validation, and test splits. It writes an immutable manifest, per-episode JSONL, and a held-out-gap report with authoritative invalid-action, exploration, and resource-efficiency metrics. It is a lifecycle and generalization measurement tool, not a performance claim.
